...
以下の例はbskyの
「日銀」
というキーワードを含む投稿を一週間ごとに集計して、
Doc2vecと呼ばれる手法で数値ベクトルに変換している。
このベクトル列をLSTMに入力すれば、もっと高い精度でグーグルトレンドの予測が可能となる。


file:dyn/examples/bsky2vec.py
#+BEGIN_SRC python -n -r -l  :tangle "dyn/examples/bsky2vec.py"
...
- Unix time とJSTの関係を調べてみよう。このプログラムでは、どこでその処理を行っているか確認
  

* 課題 

- 自分が興味ある単語のについてbskyの投稿を取得して、そのベクトル表現からgoogleトレンドを予測するLSTMを作れ。
- そのLSTMの計算結果から何がわかるか考察せよ  
